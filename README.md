...
### 1. จดโดเมน `goldpilot.app`
- ไปที่ **dash.cloudflare.com → Domain Registration → Register Domains**
- พิมพ์ `goldpilot.app` → ราคาประมาณ **$14/ปี** (~490 บาท) กด Purchase
...
- **2b. ถ้าจดที่ Porkbun**: ที่ Cloudflare กด **Add a site** → `goldpilot.app` (แผน Free) → Cloudflare จะให้ nameserver 2 ตัว → เอาไปใส่ใน Porkbun (Domain → Authoritative Nameservers) → รอ ~1–24 ชม. ให้ active

### 3. สร้าง GitHub repo แล้ว push โค้ดนี้
- สร้าง repo ใหม่ที่ **github.com/new** ชื่อ `goldpilot-site` (private หรือ public ก็ได้) — **ไม่ต้องติ๊ก** add README/gitignore
- copy คำสั่ง 2 บรรทัดที่ GitHub ให้ (`git remote add origin ...` + `git push -u origin main`) มาบอก Claude ให้รันให้ **หรือ** รันเองในโฟลเดอร์ `D:\Claude Code\goldpilot-site`

### 4. เชื่อม Cloudflare Pages
- Cloudflare dash → **Workers & Pages → Create → Pages → Connect to Git**
- เลือก repo `goldpilot-site`
- ตั้งค่า build:
  - **Framework preset**: `None`
  - **Build command**: *(เว้นว่าง)*
  - **Build output directory**: `/`
- กด **Save and Deploy** → ได้ URL ชั่วคราว `goldpilot-site.pages.dev` เช็กว่าเว็บขึ้นถูก

### 5. ผูก custom domain + subdomain แดชบอร์ด
- ในหน้า Pages project → **Custom domains → Set up a custom domain** → ใส่ `goldpilot.app` แล้ว `www.goldpilot.app` (Cloudflare เพิ่ม DNS ให้เอง)
- **`app.goldpilot.app` (แดชบอร์ดสด)**: ไปที่ `cloudflared` config เดิม (`tools/` ในโปรเจกต์บอท) เพิ่ม ingress rule:
  ```yaml
  - hostname: app.goldpilot.app
